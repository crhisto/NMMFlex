...
class test_NMMFlex_sparseness_deconvolution(unittest.TestCase):
    ...
    def _import_test_data(self):
        ...
        self.expected_proportions = pd.read_csv(
            ...
            index_col=0)
        self.bulk_expression_matrix = pd.read_csv('data/gene_exp_mixtures.csv',
                                                  index_col=0)
    ...
    @pytest.mark.skip(reason="This unit tests takes too long. "
                             "Run this manually")
    def test_simple_NMF_only_X(self):
        ...
        # I will try with the first model with only Tumor samples and three
        # cell types, since I know the accuracy will be good and I can compare
        # that with the expected proportions.
        results_normal = self.dec.run_deconvolution_multiple(
            x_matrix=self.bulk_methylation_matrix,
            y_matrix=None, z_matrix=None,
            k=4, max_iterations=10, print_limit=1)

        results_sparse = self.dec.run_deconvolution_multiple(
            x_matrix=bulk_methylation_matrix_sparse,
            y_matrix=None, z_matrix=None,
            k=4, max_iterations=10, print_limit=1)
        # 1e-07
        np.testing.assert_allclose(
            results_normal.w, results_sparse.w,
            rtol=1e-2, atol=1e-2,
            err_msg='The proportion matrices are different.', verbose=True)

        numpy.testing.assert_array_almost_equal(
            x=results_normal.w, y=results_sparse.w,
    ...
    @pytest.mark.skip(reason="This unit tests takes too long: 37 min approx."
                             " Run this manually")
    def test_simple_NMF_X_Y(self):
        ...
        # I will try with the first model with only Tumor samples and three
        # cell types, since I know the accuracy will
        ...
        # This takes ages, so I need to improve the code to run faster:
        # TODO: improve code in terms of performance.
        results_sparse = self.dec.run_deconvolution_multiple(
            x_matrix=bulk_methylation_matrix_sparse,
            y_matrix=self.bulk_expression_matrix, z_matrix=None,
            k=4, alpha=0.0001, max_iterations=2, print_limit=1)
