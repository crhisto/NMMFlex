...
class test_NMMFlex_deconvolution(unittest.TestCase):
    ...
    def __init__(self, relative_path):
        ...
        # To allow the test cases to run from different configurations of
        # main path without problems
        if 'tests' in getcwd():
    ...
    def _import_test_data(self):
        ...
        self.bulk_expression_matrix = pd.read_csv(
            self.relative_path + 'data/gene_exp_mixtures.csv', index_col=0)
    ...
    def test_run_deconvolution(self):
        ...
        results = self.dec.run_deconvolution(
            x_matrix=x_derived, k=k, delta_threshold=0.0000000001,
            max_iterations=100000)

        np.testing.assert_equal(
            np.all(results.h == 0), False,
            'The h matrix has been calculated and it is all zeros.')
        np.testing.assert_equal(
            np.all(results.w == 0), False,
            'The w matrix has been calculated and it is all zeros.')
    ...
    def test_run_deconvolution_methylation_data(self):
        ...
        results = self.dec.run_deconvolution(
            x_matrix=self.bulk_methylation_matrix.iloc[:, 0:2],
    ...
    def test_run_deconvolution_multiple_alpha(self):
        ...
        results = self.dec.run_deconvolution_multiple(
            x_matrix=self.bulk_methylation_matrix.iloc[:, 0:2],
        ...
        np.testing.assert_equal(
            ...
            'The w matrix has been calculated and it is all zeros.')
        np.testing.assert_equal(
            np.all(results.a == 0), False,
            'The a matrix has been calculated and it is all zeros.')
    ...
    def test_run_deconvolution_multiple_alpha_beta_one_model(self):
        ...
        np.testing.assert_equal(
            ...
            'The a matrix has been calculated and it is all zeros.')
        np.testing.assert_equal(
            np.all(results.b == 0), False,
            'The b matrix has been calculated and it is all zeros.')
