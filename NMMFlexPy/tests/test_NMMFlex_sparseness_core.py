...
class test_NMMFlex_sparseness_core(unittest.TestCase):
    ...
    def _create_sparce_matrix_setup(self):
        ...
        # k = 4
        w_sparse = np.array([[np.nan, 6, 0, 3],
                             [0, 3, np.nan, 2],
                             [5, 0, 1, 2],
                             [8, 1, 0, np.nan],
                             [8, 1, 0, np.nan],
        ...
        self.w = np.array([[1, 6, 0, 3],
                           ...
                           [2, 0, 0, 1],
                           [5, 1, 6, 10]])
        self.h = np.array([[3, 6, 0, 3, 5, 5],
                           [0, 3, 6, 2, 8, 9],
                           [5, 0, 1, 2, 9, 0],
                           [8, 1, 0, 11, 3, 2]])

        self.a = np.array([[2, 3, 6, 7],
                           [0, 0, 6, 2],
                           [2, 0, 1, 4],
                           [6, 1, 0, 2],
                           [5, 6, 0, 5],
        ...
        # Initialization of x to be sparse with nulls because it is already
        # sparce in zeros.
    ...
    def test_calculate_calculate_divergence_sparse(self):
        ...
        divergence_matrix_sparse = self.dec._calculate_divergence_generic(
            self.x_sparse, x_hat, True).astype(float)

        # As in the core function, I calculate the divergence_sum with np.sum
        # function.
        sum_divergence = np.sum(divergence_matrix_sparse)
        print('Divergence sum: ', sum_divergence)
        divergence_matrix_normal = pd.DataFrame.sparse.from_spmatrix(
            divergence_matrix_sparse)

        self.assertEqual(True,
                         divergence_matrix_normal is not None,
                         'The divergence_matrix_sparsity is not calculated.')
        self.assertGreater(
            sum_divergence,
            0,
            'Unexpected negative value for the sum of the divergence.')

    def test_calculate_calculate_divergence_not_sparse_equal_matrices(self):
        # Let's take a matrix with different values based on the original one.
        # In this case the matrices are not sparse.
        x_hat = self.x * 1
        ...
        divergence_matrix_sparse = self.dec._calculate_divergence_generic(
            ...
            x_hat,
            True).astype(float)
        divergence_matrix_not_sparse = self.dec._calculate_divergence_generic(
            self.x,
            x_hat,
            True).astype(float)
        ...
        self.assertEqual(True, divergence_matrix_normal is not None,
                         'The divergence_matrix_sparsity is not calculated.')
        self.assertEqual(sum_divergence_sparse,
                         0,
                         'Different values for the sum of the sparse '
                         'divergence. It should be zero.')
        self.assertEqual(sum_divergence_not_sparse, 0,
                         'Different values for the sum of the not sparse '
                         'divergence. It should be zero.')
    ...
    def test_calculate_h_new_extended_alpha_beta_sparse_only_x(self):
        ...
        h_new = self.dec._calculate_h_new_extended_alpha_beta_generic(
            self.x_sparse, x_hat, self.w, self.h,
            alpha, y, y_hat, a,
            proportion_constraint, is_model_sparse)
        print(h_new)

        sparsity_nulls = self.dec.sparsity_calculation(h_new,
                                                       type_analysis='nulls')

        self.assertEqual(
            True, h_new is not None,
            'The matrix H is calculate based on a X sparse matrix.')
        self.assertEqual(0.0,
                         sparsity_nulls,
                         'The H matrix has nulls, therefore some went wrong')
    ...
    def test_calculate_h_new_extended_alpha_beta_sparse_x_y(self):
        ...
        self.assertEqual(
            True, h_new is not None,
            'The matrix H is calculate based on a X and Y sparse matrix.')
        self.assertEqual(0.0, sparsity_nulls,
                         'The H matrix does not have nulls')
    ...
    def test_calculate_a_new_extended_sparse(self):
        ...
        a_new = self.dec._calculate_a_new_extended_generic(
            self.y_sparse, y_hat, self.a, self.h, is_model_sparse)
        print(a_new)

        sparsity_nulls = self.dec.sparsity_calculation(a_new,
                                                       type_analysis='nulls')

        self.assertEqual(
            True, a_new is not None,
            'The matrix A is calculate based on a Y sparse matrix.')
        self.assertEqual(0.0, sparsity_nulls,
                         'The A matrix does not have nulls.')
    ...
    def test_calculate_b_new_extended_sparse(self):
        ...
        b_new = self.dec._calculate_b_new_extended_generic(
            self.z_sparse, z_hat, self.b, self.w, is_model_sparse)
        print(b_new)

        sparsity_nulls = self.dec.sparsity_calculation(b_new,
                                                       type_analysis='nulls')

        self.assertEqual(
            True, b_new is not None,
            'The matrix B is calculate based on a Z sparse matrix.')
        self.assertEqual(0.0, sparsity_nulls,
                         'The B matrix does not have nulls.')
    ...
    def test_calculate_w_new_extended_alpha_beta_sparse_only_x(self):
        ...
        w_new = self.dec._calculate_w_new_extended_alpha_beta_generic(
            self.x_sparse, x_hat, self.w, self.h, beta, z,
            z_hat, b, regularize_w, alpha_regularizer_w,
            is_model_sparse)
        print(w_new)

        sparsity_nulls = self.dec.sparsity_calculation(w_new,
                                                       type_analysis='nulls')

        self.assertEqual(
            True, w_new is not None,
            'The matrix W has not been calculate based on a X sparse matrix.')
    ...
    def test_calculate_w_new_extended_alpha_beta_sparse_x_z(self):
        ...
        w_new = self.dec._calculate_w_new_extended_alpha_beta_generic(
            self.x_sparse, x_hat, self.w, self.h, beta,
