...
class test_NMMFlex_basics(unittest.TestCase):
    """
    The test_NMMFlex_basics class is a TestCase subclass for testing the basics of NMMFlex.

    Attributes:
    bulk_methylation_matrix (numpy.ndarray or None): The input bulk methylation matrix for testing. Default is None.
    expected_proportions (numpy.ndarray or None): The expected proportions for the tests cases. Default is None.
    bulk_expression_matrix (numpy.ndarray or None): The input bulk expression matrix for testing. Default is None.
    data_expression_auxiliary_matrix_random (numpy.ndarray or None): The random data expression auxiliary matrix for
    testing. Default is None.
    dec (Deconvolution or None): The Deconvolution instance to be used for testing. Default is None.
    ...
    - The setUp method can be used to set up any state that is common to all tests methods.
    - The tearDown method can be used to clean up any resources after tests are run.
    """
    ...
    def test_calculate_x_hat(self):
        w_test = np.array([[3, 6, 7, 3], [5, 3, 0, 2], [5, 2, 1, 2], [8, 1, 0, 10]])  # k = 4
        ...
        print(x_hat)
        np.testing.assert_array_equal(x_hat_expected, x_hat, 'The X^ (x hat) matrix is not the one expected.')
    ...
    def test_calculate_x_hat_extended(self):
        ...
        x_hat = self.dec._calculate_x_hat_extended(w_test, h_test).astype(float)
        print('X^ (x hat) matrix calculated with extended code:')
        print(x_hat)
        print('X^ (x hat) matrix calculated with dot function:')
        x_hat_dot_product = self.dec._calculate_x_hat(w_test, h_test).astype(float)
        print(x_hat_dot_product)
        np.testing.assert_array_equal(x_hat_dot_product, x_hat, 'The X^ (x hat) matrix is not equal to the dot prod".')
        np.testing.assert_array_equal(x_hat_expected, x_hat, 'The X^ (x hat) matrix is not the one expected.')
    ...
    def test_calculate_w_new_extended(self):
        ...
        w = np.array([[43, 4, 3, 3], [1, 9, 2, 1], [2, 9, 2, 1], [3, 11, 7, 2]])
        w_new_expected = np.array([[38.9, 3.8, 2.8, 2.8], [0.9, 8.1, 1.8, 0.9], [1.8, 6.9, 1.5, 0.8],
                                   [3.7, 15.6, 9.9, 2.8]])

        h_test = np.array([[3, 6], [5, 3], [5, 3], [5, 3]])

        w_new = self.dec._calculate_w_new_extended(x_test, x_hat_test, w, h_test)
        print("W_new matrix: ")
        print(w_new)
        # I want to see if I can get the matrix with a decimal.
        np.testing.assert_almost_equal(w_new_expected, w_new, 1, 'The new w matrix is not the one expected.', )
    ...
    def test_proportion_constraint_h(self):
        ...
        h_expected = np.array([[0.112094489, 0.4844611, 0.026351282],
        ...
        np.testing.assert_allclose(h_expected, h_new, 1e-7, 0, 'The matrix H normalize is not the expected.')
        self.assertEqual(3, h_new_sum, 'Each column sums 1 in the H matrix (proportion matrix)')
    ...
    def test_calculate_divergence_equal_matrix(self):
        ...
        divergence_matrix = self.dec._calculate_divergence_generic(x, x_hat, False)
        print(divergence_matrix)
        divergence_value = np.sum(divergence_matrix)
        self.assertEqual(0.0, divergence_value, 'The divergence sum is not the expected.')
    ...
    def test_divergence_extended(self):
        ...
        divergence_matrix_dot_method = self.dec._calculate_divergence_generic(x, x_hat, False).astype(float)
        divergence_matrix_extended_method = self.dec._calculate_divergence_extended(x, x_hat).astype(float)

        np.testing.assert_array_equal(divergence_matrix_dot_method, divergence_matrix_extended_method,
                                      'The matrix created with dot method is different to the one created with the '
                                      'extended method')

    def test_normalization(self):
        matrix_quantile_normalization = self.dec.normalization(matrix=self.bulk_methylation_matrix,
                                                               normalization_type='quantile_norm')

        matrix_min_max = self.dec.normalization(matrix=matrix_quantile_normalization,
                                                normalization_type='norm_zero_min_max')

        matrix_quantile_normalization_min_max = self.dec.normalization(matrix=self.bulk_methylation_matrix,
                                                                       normalization_type='quantile_norm_min_max')

        print(type(matrix_quantile_normalization))
        print("pandas.core.frame.DataFrame" == type(matrix_quantile_normalization))
        print(isinstance(matrix_quantile_normalization, pandas.core.frame.DataFrame))
